...
Understanding *how* a deep learning model arrives at its classification is crucial for clinical trust and adoption, moving beyond simple accuracy metrics. To gain insights into the decision-making process of the R3D-18 model, Gradient-weighted Class Activation Mapping (Grad-CAM) was employed. Grad-CAM produces saliency maps highlighting the input regions most influential in determining the model's output for a specific class~\cite{selvaraju2017grad}. Figure \ref{fig:gradcam} presents these visualizations for representative AD and CN subjects from the test set, showcasing activation patterns across different sagittal slices for both correct and incorrect classifications.

Analysis of the Grad-CAM maps for correctly classified AD subjects (Row 1: "Ad Correct") reveals relatively consistent patterns of high activation (indicated by red/yellow areas) primarily localized in regions known to be affected early and significantly by Alzheimer's disease. Across different subjects and slices, the model frequently focuses on the temporal lobes, including the hippocampus and surrounding structures, as well as areas around the ventricles, which often enlarge due to brain atrophy. For instance, Subject 1 shows strong, widespread activation across the temporal and parietal regions, particularly evident in slices 65 and 75. Subject 3 shows similar, though perhaps more focal, activation in the temporal lobe and near the ventricles. This alignment between model attention and known AD pathology suggests the model has learned to identify relevant structural biomarkers.

Conversely, for correctly classified CN subjects (Row 2: "Cn Correct"), the activation patterns are generally more diffuse and less intense. While some areas show moderate activation, there isn't the same consistent focus on specific neuroanatomical regions associated with AD pathology. The model appears to rely on a broader assessment or the absence of strong AD-like features, rather than identifying specific "healthy" markers. This is expected, as the task is framed around detecting the presence of AD patterns.

Examining the misclassified cases provides further insight. For AD subjects incorrectly classified as CN (Row 3: "Ad Incorrect" - False Negatives), the Grad-CAM maps often show weaker or misdirected activation. For example, Subject 1 (Incorrect AD) shows some activation, but it's less intense and widespread compared to the correctly classified AD cases. Subjects 2 and 3 (Incorrect AD) exhibit focal activation primarily concentrated deep within the brain, potentially near the brainstem or thalamus, rather than the typical cortical or hippocampal regions. This suggests the model may fail when AD pathology presents atypically or when the characteristic structural changes are less pronounced in the specific slices examined.

For CN subjects incorrectly classified as AD (Row 4: "Cn Incorrect" - False Positives), the model sometimes highlights regions that might exhibit age-related changes confusable with early AD. Subject 3 (Incorrect CN), slice 75, shows activation in the temporal lobe area, potentially indicating that normal anatomical variations or age-related atrophy in this individual were misinterpreted by the model as pathological. Subject 2 shows some diffuse posterior activation. Often, however, the activation patterns in false positive cases appear less distinct or anatomically specific than in true positive AD cases, suggesting the model might be latching onto less reliable or more ambiguous features.

Overall, the Grad-CAM visualizations indicate that the R3D-18 model, despite being trained via transfer learning from a non-medical domain, has learned to associate specific brain regions, particularly the temporal lobes and periventricular areas, with an AD diagnosis. The consistency of activation in these regions for correct AD classifications lends credibility to the model's approach. However, the variability in activation patterns, especially in misclassified cases, highlights the challenges in distinguishing subtle AD patterns from normal aging or atypical presentations. While Grad-CAM provides valuable qualitative insights, it is a relatively low-resolution technique and doesn't fully elucidate the complex interplay of features the model uses. Future work could employ more advanced XAI methods to gain a deeper, more quantitative understanding of the model's learned representations and decision boundaries.
